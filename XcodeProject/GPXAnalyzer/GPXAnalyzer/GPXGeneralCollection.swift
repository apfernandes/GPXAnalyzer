         ...
         do {
            let fileURLs = try FileManager.default.contentsOfDirectory(at: folderURL, includingPropertiesForKeys: nil)
             ...
             DispatchQueue.global(qos: .userInitiated).async { [unowned self] in
                 ...
                 HTMLText! += "background-color: black;"
                 HTMLText! += "}"
                 HTMLText! += "</style>"
                 HTMLText! += "</head>"
                 HTMLText! += "<body>"
                ...
                HTMLText! += "<pre>"
                HTMLText! += "Activity Type\tDate\tYear\tDay of The Year\tAccumulated Distance in km\tTotal Distance in km\tTotal time in seconds\n"
                 
                print (HTMLText!)
                 
                for gpx in self.arrayOFGPXFiles! {
                    
                    var nextLine = "\(gpx.summaryString!)"
                    print (nextLine)
                    nextLine += "\n"
                    HTMLText! += nextLine
                ...
                HTMLText! += "</body>"
                 ...
                 delegate.finishedProcessingGPXFiles()
